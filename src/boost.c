...
void init_boosts(void) {
    ...
    set_pex_pin_dir(&pex, PEX_A, 10VBST_ENBL, OUTPUT);
    set_pex_pin(&pex, PEX_A, 10VBST_ENBL, 0);

    // set 6V boost associated pins
    set_pex_pin_dir(&pex, PEX_B, 6VBST_ENBL, OUTPUT);
    set_pex_pin(&pex, PEX_B, 6VBST_ENBL, 0);
}

void enable_10V_boost(void) {
    // Enable 10V boost converter by setting corresponding pex pin HIGH
    set_pex_pin(&pex, PEX_A, 10VBST_ENBL, 1);
}
...
void enable_6V_boost(void) {
    ...
    set_pex_pin(&pex, PEX_B, 6VBST_ENBL, 1);
}
